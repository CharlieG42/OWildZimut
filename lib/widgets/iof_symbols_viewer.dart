  ...
  Widget _buildGeometryDetails(IOFSymbol symbol) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        if (geomType == 'point') ...[
          Text('Inner Radius: ${props['inner_radius']?.toString()} (${((int.tryParse(props['inner_radius']?.toString() ?? '0') ?? 0) / 1000.0).toStringAsFixed(2)}mm)'),
          Text('Inner Color: ${props['inner_color']?.toString()}'),
          Text('Outer Color: ${props['outer_color']?.toString()}'),
          Text('Rotatable: ${props['rotatable']?.toString()}'),
        ],
        if (geomType == 'line') ...[
          Text('Color: ${props['color']?.toString()}'),
          Text('Line Width: ${props['line_width']?.toString()} (${((int.tryParse(props['line_width']?.toString() ?? '0') ?? 0) / 1000.0).toStringAsFixed(2)}mm)'),
          Text('Dashed: ${props['dashed']?.toString()}'),
          if (props['dash_length'] != null) Text('Dash Length: ${props['dash_length']?.toString()}'),
          if (props['break_length'] != null) Text('Break Length: ${props['break_length']?.toString()}'),
          if (props['join_style'] != null) Text('Join Style: ${props['join_style']?.toString()}'),
          if (props['cap_style'] != null) Text('Cap Style: ${props['cap_style']?.toString()}'),
        ],
        if (geomType == 'area') ...[
          Text('Inner Color: ${props['inner_color']?.toString()}'),
          Text('Rotatable: ${props['rotatable']?.toString()}'),
        ...
        if (geomType == 'text') ...[
          Text('Color: ${props['color']?.toString()}'),
          Text('Font Size: ${props['font_size']?.toString()}'),
          Text('Rotatable: ${props['rotatable']?.toString()}'),
        ],
      ],
    );
